...
import { Download, Ticket, BarChart3, TicketPercent, DollarSign } from 'lucide-react';
import {
  Select,
  SelectContent,
  SelectItem,
...
import { ticketTypeSalesData, upcomingEvents } from '@/lib/data';
...
export default function TicketTypeSalesPage() {
  ...
  const stats = React.useMemo(() => {
    ...
    const totalTicketsSold = filteredSales.reduce((sum, sale) => sum + sale.ticketsSold, 0);
    const uniqueTicketTypes = new Set(filteredSales.map(sale => sale.ticketType)).size;
    return { totalRevenue, totalTicketsSold, uniqueTicketTypes };
  }, [filteredSales]);

  const chartData = React.useMemo(() => {
    const dataByTicketType = filteredSales.reduce((acc, sale) => {
  ...
  const handleExport = () => {
    ...
    const headers = ['Event', 'Ticket Type', 'Price', 'Tickets Sold', 'Revenue'];
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Ticket Type Sales</h1>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-5">
        <Card className="lg:col-span-3">
          <CardHeader>
            ...
            <CardDescription>
              A comprehensive list of sales per ticket type for the selected event(s).
            </CardDescription>
          </CardHeader>
          <CardContent>
            <Table>
              ...
              <TableBody>
                {filteredSales.length > 0 ? (
                  filteredSales.map((sale, index) => (
                    <TableRow key={`${sale.event}-${sale.ticketType}-${index}`}>
                      ...
                      <TableCell className="text-right">${sale.revenue.toLocaleString('en-US', { minimumFractionDigits: 2 })}</TableCell>
                    </TableRow>
                  ))
                ) : (
                  <TableRow>
        ...
        <Card className="lg:col-span-2">
          <CardHeader>
            ...
            <CardDescription>
              A visual breakdown of revenue contributions from each ticket type.
            </CardDescription>
          </CardHeader>
          <CardContent>
             {chartData.length > 0 ? (
                <SalesByTypeChart data={chartData} />
                 ...
                 <div className="flex h-[300px] flex-col items-center justify-center rounded-lg border-2 border-dashed bg-muted/20">
                    ...
                    <h3 className="text-xl font-semibold text-muted-foreground">No Chart Data</h3>
